...
|  Field Name | Type | Presence | Description |
|  ------ | ------ | ------ | ------ |
|  `from_stop_id` | Foreign ID referencing `stops.stop_id` | **Conditionally Required** | Identifies a stop or station where a connection between routes begins. If this field refers to a station, the transfer rule applies to all its child stops. <br><br>Conditionally Required:<br>- **Required** for transfer points between routes (`transfer_type=0`), timed transfers (`transfer_type=1`), minimal transfer times (`transfer_type=3`)<br>- Optional for in-seat transfers (`transfer_type=4`) and disallowed in-seat transfers (`transfer_type=5`)<br><br>Refering to a station is forbiden for `transfer_types` 4 and 5.|
|  `to_stop_id` | Foreign ID referencing `stops.stop_id` | **Conditionally Required** | Identifies a stop or station where a connection between routes ends. If this field refers to a station, the transfer rule applies to all child stops. <br><br>Conditionally Required:<br>- **Required** for transfer points between routes (`transfer_type=0`), timed transfers (`transfer_type=1`), minimal transfer times (`transfer_type=3`)<br>- Optional for in-seat transfers (`transfer_type=4`) and disallowed in-seat transfers (`transfer_type=5`)<br><br>Refering to a station is forbiden for `transfer_types` 4 and 5.|
